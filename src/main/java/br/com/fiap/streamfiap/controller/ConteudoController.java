...
public class ConteudoController {
    ...
    public ResponseEntity<Filme> cadastrarFilme(@RequestBody Filme filme) {
    	if (filme.getDuracaoMinutos() <= 0) throw new IllegalArgumentException("Duração inválida");
     ...
     // Troque 'filme' por 'serie' ou 'documentario' nos outros métodos
    }

    // POST /api/conteudos/serie - cadastra uma série
    @PostMapping("/serie")
    public ResponseEntity<Serie> cadastrarSerie(@RequestBody Serie serie) {
    	if (serie.getDuracaoMinutos() <= 0) throw new IllegalArgumentException("Duração inválida");
    ...
    public ResponseEntity<Documentario> cadastrarDocumentario(@RequestBody Documentario documentario) {
    	if (documentario.getDuracaoMinutos() <= 0) throw new IllegalArgumentException("Duração inválida");
